...
module.exports = {
  post(req, reply) {
    ...
    Joi.validate(req.payload, extensionSchema, err => {
      ...
      for (const batchRequest of req.payload.requests) {
        if (batchRequest.action !== 'delete' && indexObjectVerifier.exceedsSizeLimit(batchRequest.body)) {
          return reply(Boom.entityTooLarge(`Object in request ${i} exceeds 10k size limit`));
        }
