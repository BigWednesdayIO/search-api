...
class Index {
  ...
  batchOperation(operations) {
    ...
    const inserts = operations.insert || [];

    if (inserts.length) {
      const insertHeader = {index: {_index: this._indexName, _type: objectTypeInIndex}};

      inserts.forEach(o => {
        bulkArgs.body.push(insertHeader);
        bulkArgs.body.push(o);
      });
    }

    const upserts = operations.upsert || [];
    upserts.forEach(o => {
      bulkArgs.body.push({index: {_index: this._indexName, _type: objectTypeInIndex, _id: o.objectID}});
      bulkArgs.body.push(o.data);
    });

    return elasticsearchClient.bulk(bulkArgs)
      .then(res => {
        return {
          inserted: _.map(res.items.slice(0, inserts.length), 'create._id'),
          upserted: _.map(res.items.slice(inserts.length, upserts.length), 'index._id')
        };
      });
  }
}
