  ...
  saveSettings(settings) {
    return createIndexIfNotExists(this._indexName)
      ...
      .then(currentMapping => {
        ...
        const currentIndexMapping = currentMapping ? currentMapping[Object.keys(currentMapping)[0]].mappings : null;
        const currentMappingProperties = currentIndexMapping ? currentIndexMapping.properties : {};
        ...
        if (searchableFields.length) {
          // when fields are configured, disable any additional fields that might be added later
        ...
        // set properties to a default value of analyzed if no fields are configured, otherwise di
        ...
        searchableFields.forEach(field => {
          if (mapping[objectTypeInIndex].properties[field]) {
            // ensure existing field is searchable
            mapping[objectTypeInIndex].properties[field].index = 'analyzed';
          } else {
            // anticipate new field creation by defining a template for it
            const fieldTemplate = {
              [field]: {
                match: field,
                mapping: {
  ...
  getSettings() {
    return elasticsearchClient.indices.getMapping({index: this._indexName, type: objectTypeInIndex})
      .then(mapping => {
        ...
        const objectMapping = mapping[Object.keys(mapping)[0]].mappings[objectTypeInIndex];

        _.forEach(objectMapping.properties, (property, key) => {
          if (property.index !== 'no') {
            searchable_fields.push(key);
          }
        });

        objectMapping.dynamic_templates.forEach(template => {
          const templateName = Object.keys(template)[0];

          if (templateName !== 'not_searchable') {
            searchable_fields.push(templateName);
          }
        });
