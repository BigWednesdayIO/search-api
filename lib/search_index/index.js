...
const mapResults = function (results, settings) {
  return {
    hits: _.map(results.hits.hits, hit => {
      return _.assign(hit._source, {objectID: hit._id});
    }),
    facets: _(results.aggregations)
      .map((aggregation, key) => {
        const order = _.find(settings.facets, {field: key}).order;

        const values = _(aggregation.buckets)
          .map(b => {
            return {value: b.key, count: b.doc_count};
          })
          .sortBy(facetValue => {
            return order.indexOf('value') === 0 ? facetValue.value.toLowerCase() : facetValue.count;
          })
          .value();
      ...
      .sortBy(facet => _.findIndex(settings.facets, {field: facet.field}))
      .value(),
...
class SearchIndex {
  ...
  getManyObjects(objectIDs) {
    return elasticsearchClient.mget({
      index: this._indexName,
      type: objectTypeInIndex,
      body: {
        ids: objectIDs
      ...
      const results = response.docs
        ...
        .map(doc => Object.assign({objectID: doc._id}, doc._source));

      if (results.length === 0 && response.docs[0].error && response.docs[0].error.type === 'index_not_found_exception') {
  ...
  query(body) {
    return elasticsearchClient.indices.getMapping({index: this._indexName, type: objectTypeInIndex})
      .then(mapping => {
        return {
          settings: extractSettingsFromMapping(mapping) || buildDefaultSettings(mapping),
          mapping: extractObjectMapping(mapping)
        };
      })
      .then(context => {
        if (!context.settings) {
        ...
        return elasticsearchClient.search({
          ...
          body: queryBuilder.build(body, context)
        })
        .then(results => mapResults(results, context.settings));
      })
      .catch(err => {
        if (err.status === 404) {
          throw new IndexNotFoundError('Index not found');
