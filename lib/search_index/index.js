...
class SearchIndex {
  ...
  move(destinationIndex) {
    return elasticsearchClient.indices.getAlias({name: [destinationIndex._indexName, this._indexName]})
      .then(indexes => {
        ...
        return {
          source: _.find(indexes, {alias: this._indexName}),
          destination: _.find(indexes, {alias: destinationIndex._indexName}) || {alias: destinationIndex._indexName}
        };
      })
      .then(indexes => {
