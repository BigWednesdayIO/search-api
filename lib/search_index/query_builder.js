...
const buildTermsFilter = function (filterParams, mapping) {
  const terms = {[getUntokenizedField(filterParams.field, mapping)]: filterParams.terms};
  return {terms};
};
...
const buildFilter = function (filterParams, mapping) {
  ...
  if (filterParams.terms) {
    return buildTermsFilter(filterParams, mapping);
  }
