...
const buildFacetAggregation = function (field, order) {
  const orderKey = order.indexOf('value') === 0 ? '_term' : '_count';
  const orderDirection = order.indexOf('DESC') > 0 ? 'desc' : 'asc';

  return {terms: {field, size: 0, order: {[orderKey]: orderDirection}}};
};
...
const buildQuery = function (searchParams, context) {
  ...
  if (settings.facets) {
    ...
    settings.facets.forEach(facet => {
      query.aggregations[facet.field] = buildFacetAggregation(getUntokenizedField(facet.field, mapping), facet.order);
    });
  }
